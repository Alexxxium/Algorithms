...
/*
    ...
    Причем поиск всегда будет бинарным: (с погрешностью в одну итерацию) O(log(N)) и в худшем случае O(N/2)+1.
...
struct Pair
{
    ...
    Pair(int vl = 0): first(vl), second(false) {}
};
...
const T& binarySearch(std::vector<T> &vec, T  &&vl)
{
    ...
    while (left < right)                        // фиксируем подозреваемый индекс (движимся с приоритетом влево)
    {
        if (vl > vec[mid])
            left = mid + 1;
        else                                    // может меньше, а может и равный (тогда просто ищем самый первый такой элемент)
            right = mid - 1;

        mid = (left + right) / 2;
    }
    // На выходе либо текущий элемент искомый (левый), либо следующий (правый) 
    if (vec[right] == vl)
        return vec[right];                      
